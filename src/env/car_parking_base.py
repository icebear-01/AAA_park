...
class CarParking(gym.Env):
    ...
    def _get_reward(self, prev_state: State, curr_state: State):
        ...
        if REWARD_WEIGHT['rs_dist_reward'] != 0:
            ...
            curr_rs_path = rsCurve.calc_optimal_path(*curr_state.get_pos(), *self.map.dest.get_pos(), radius , 0.1)
            prev_rs_path = rsCurve.calc_optimal_path(*prev_state.get_pos(), *self.map.dest.get_pos(), radius, 0.1)
            norm_rs_path = rsCurve.calc_optimal_path(*self.map.start.get_pos(), *self.map.dest.get_pos(), radius, 0.1)
            if curr_rs_path is None or prev_rs_path is None or norm_rs_path is None:
                rs_dist_reward = 0
            else:
                curr_rs_dist = curr_rs_path.L
                prev_rs_dist = prev_rs_path.L
                rs_dist_norm_ratio = norm_rs_path.L
    ...
    def step(self, action:np.ndarray = None):
        ...
        info = OrderedDict({'reward_info':reward_info,
            'path_to_dest':None})
        if self.enable_rs_assist and self.t > 1 and status==Status.CONTINUE\
    ...
    def find_rs_path(self,status):
        ...
        '''
        return self.find_rs_path_to_state(self.map.dest)

    def find_rs_path_to_state(self, goal_state, start_state=None):
        if start_state is None:
            start_state = self.vehicle.state

        startX, startY, startYaw = start_state.loc.x, start_state.loc.y, start_state.heading
        goalX, goalY, goalYaw = goal_state.loc.x, goal_state.loc.y, goal_state.heading
        radius = math.tan(VALID_STEER[-1])/WHEEL_BASE
