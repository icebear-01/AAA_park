...
class PPOAgent(AgentBase):
    ...
    def _actor_forward(self, obs) -> torch.distributions.Distribution: # to be replaced
        ...
        if self.configs.state_norm:
            if isinstance(observation, list):
                observation = [self.state_normalize.state_norm(o) for o in observation]
            else:
    ...
    def choose_actions(self, obs_list):
        dist = self._actor_forward(obs_list)
        if self.discrete:
            raise NotImplementedError("Batched action sampling is only implemented for continuous policies")

        action_masks = [obs['action_mask'] for obs in obs_list]
        mean_batch = dist.mean
        std_batch = dist.stddev
        sampled_action_batch = dist.sample()
        action_tensors = []

        for idx, action_mask in enumerate(action_masks):
            if action_mask is not None:
                action = self.action_filter.choose_action(mean_batch[idx], std_batch[idx], action_mask)
                action_tensor = torch.FloatTensor(action).to(self.device)
            else:
                action_tensor = sampled_action_batch[idx]
                if self.configs.dist_type == "gaussian":
                    action_tensor = torch.clamp(action_tensor, -1, 1)
            action_tensors.append(action_tensor)

        action_batch = torch.stack(action_tensors, dim=0)
        log_prob_batch = dist.log_prob(action_batch)
        return action_batch.detach().cpu().numpy(), log_prob_batch.detach().cpu().numpy()
